...
namespace EPPlusTest
{
    ...
    public class Issues : TestBase
    {
        ...
        [TestMethod]
        public void DependecyChainIssue()
        {
            using (var p = OpenTemplatePackage("simple if.xlsx"))
            {
                SetInputSheetValues(p);
                SetLocationSheetValues(p);
                SetCurrencySheetValues(p);
                var ws = p.Workbook.Worksheets["Calc core rate"];
                var f = ws.Cells["A4"].Formula;
                var c = ws.Cells["A4"].Value;
                ws.Cells["A4"].Formula = c.ToString().TrimStart('=');
                for (var x = 0; x < 4; x++)
                { 
                    p.Workbook.Calculate();

                    Debug.WriteLine("Q4:" + ws.Cells["Q4"].Value);
                    Debug.WriteLine("R4:" + ws.Cells["R4"].Value);
                    Debug.WriteLine("T4:" + ws.Cells["T4"].Value);
                    Debug.WriteLine("U4:" + ws.Cells["U4"].Value);
                    Debug.WriteLine("V4:" + ws.Cells["V4"].Value);
                }
            }
        }
        private void SetCurrencySheetValues(ExcelPackage excelPackage)
        {
            var currencyWorksheet = excelPackage.Workbook.Worksheets["INPUT_CURRENCY_CONVERSION"];
            for (var i = 1; i < 20; i++)
            {
                currencyWorksheet.SetValue($"C{i + 1}", i);
                currencyWorksheet.SetValue($"D{i + 1}", i);
            }
        }

        private void SetLocationSheetValues(ExcelPackage excelPackage)
        {
            var locationWorksheet = excelPackage.Workbook.Worksheets["INPUT_LOCATION_BREAKDOWN"];
            locationWorksheet.SetValue($"A2", "ZAF");
            locationWorksheet.SetValue($"B2", "EC");
            locationWorksheet.SetValue($"C2", "ZAF_EC");
            locationWorksheet.SetValue($"D2", 1);
        }

        private static void SetInputSheetValues(ExcelPackage excelPackage)
        {
            var inputWorksheet = excelPackage.Workbook.Worksheets["INPUT"];
            inputWorksheet.SetValue("C2", DateTime.Parse("2021/02/17"));
            inputWorksheet.SetValue("C3", "Female");
            inputWorksheet.SetValue("C4", DateTime.Parse("1964/02/17"));
            inputWorksheet.SetValue("C5", 60000);
            inputWorksheet.SetValue("C6", "ZAF");
            inputWorksheet.SetValue("C7", "ZAR");
            inputWorksheet.SetValue("C8", "ZAR");
            inputWorksheet.SetValue("C9", "D");
        }
    }
}