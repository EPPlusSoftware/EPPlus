 ...
 *************************************************************************************************
  Date               Author                       Change
 *************************************************************************************************
  01/12/2026         EPPlus Software AB           GPOS Lookup Type 1 Format 1 Deserializer
 *************************************************************************************************/
...
{
    /// <summary>
    /// Deserializer for GPOS Lookup Type 1, Format 1: Single Adjustment Positioning
    /// </summary>
    internal class SinglePosSubTableFormat1Deserializer
    {
        private readonly FontsBinaryReader _reader;

        public SinglePosSubTableFormat1Deserializer(FontsBinaryReader reader)
        {
            _reader = reader;
        }

        /// <summary>
        /// Deserializes a SinglePos Format 1 subtable from the current stream position.
        /// </summary>
        /// <param name="subtableStartOffset">Absolute offset where this subtable starts</param>
        /// <returns>Deserialized SinglePosSubTableFormat1</returns>
        public SinglePosSubTableFormat1 Deserialize(long subtableStartOffset)
        {
            _reader.BaseStream.Seek(subtableStartOffset, SeekOrigin.Begin);

            var subtable = new SinglePosSubTableFormat1
            {
            ...
            // Read the single ValueRecord
            subtable.Value = ReadValueRecord(subtable.ValueFormat);
