...
namespace OfficeOpenXml.Utils.AttributesUtils
{
    ...
    {
        internal static bool HasAttributeOfType<T>(this MemberInfo member, bool? inherit = default)
            where T : Attribute
        {
            return member.HasAttributeOfType(out T attr, inherit);
        }
        internal static bool HasAttributeOfType<T>(this MemberInfo member, out T attr, bool? inherit = default)
            where T : Attribute
        {
            attr = default;
#if (NET35 || NET40)
            attr = member.GetCustomAttributes(typeof(T), inherit ?? false).FirstOrDefault() as T;
#else

            if (!inherit.HasValue)
            {
                attr = member.GetCustomAttributes(typeof(T)).FirstOrDefault() as T;
            }
            else
            {
                attr = member.GetCustomAttributes(typeof(T), inherit.Value).FirstOrDefault() as T;
            }
#endif
            return attr != null;
        }

        internal static T GetFirstAttributeOfType<T>(this MemberInfo member, bool? inherit = default)
            where T : Attribute
        {
#if (NET35 || NET40)
