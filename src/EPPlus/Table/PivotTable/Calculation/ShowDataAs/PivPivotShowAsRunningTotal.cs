...
namespace OfficeOpenXml.Table.PivotTable.Calculation.ShowDataAs
{
    internal class PivotShowAsRunningTotal : PivotShowAsBase
    {
        internal override void Calculate(ExcelPivotTableDataField df, List<int> fieldIndex, ref PivotCalculationStore calculatedItems)
        {
            ...
            var prevValue = 0D;
            var prevKey = PivotCalculationStore.SumLevelValue;
            foreach (var key in calculatedItems.Index.OrderBy(x => x.Key, ArrayComparer.Instance).Where(x => x.Key[0]!= PivotCalculationStore.SumLevelValue))
            {
                if (IsSumBefore(key.Key, bf, fieldIndex, colFieldsStart) || key.Key[bf] == PivotCalculationStore.SumLevelValue)
                {
