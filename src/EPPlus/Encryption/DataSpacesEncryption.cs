    ...
    internal class DataSpacesEncryption
    {
        internal static SensibilityLabelInfo ReadDataSpaceEnrcyptionInfo(CompoundDocument doc)
        {
            try
            {
                ...
                //Root Streams
                si.SummaryInfoProperties = ReadEncryptedPropertyStreamInfo(doc.Storage.DataStreams["\u0005SummaryInformation"].Stream);

                si.SummaryDocumentInfoProperties = ReadEncryptedPropertyStreamInfo(doc.Storage.DataStreams["\u0005DocumentSummaryInformation"].Stream);
        ...
        private static TransformInfoHeader ReadTransformReferences(CompoundDocument.StoragePart dsStorage)
        {
            ...
            using (var ms = new MemoryStream(streamBytes.Stream))
            {
                var br = new BinaryReader(ms);
