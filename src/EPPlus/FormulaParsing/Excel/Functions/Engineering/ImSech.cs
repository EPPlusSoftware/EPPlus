...
namespace OfficeOpenXml.FormulaParsing.Excel.Functions.Engineering
{
    ...
    internal class ImSech : ImFunctionBase
    {
        ...
        public override CompileResult Execute(IList<FunctionArgument> arguments, ParsingContext context)
        {
            ...
            var realNumerator = 2 * Math.Cosh(real) * Math.Cos(imag);
            var realDenominator = Math.Cosh(2 * real) + Math.Cos(2 * imag);
            var realPart = realNumerator / realDenominator;

            var imagNumerator = 2 * Math.Sinh(real) * Math.Sin(imag);
            var imagDenominator = Math.Cosh(2 * real) + Math.Cos(2 * imag);
