    ...
    internal class RichValueErrorManager
    {
        ...
        internal void SetMetaDataForError(CellStoreEnumerator<ExcelValue> cse, ExcelErrorValue error)
        {
            ...
            if(newRv != null)
            {
                ...
                var metadataRef = _ws._metadataStore.GetValue(cse.Row, cse.Column);
                metadataRef.vm = vmId;
                _ws._metadataStore.SetValue(cse.Row, cse.Column, metadataRef);
            }
        }
        ...
        private ErrorPropagatedRichValue CreatePropagated(eErrorType errorType)
        {
            ...
            item.SetStructure(_ws.Workbook.RichData.Db);
            return item;
        }
