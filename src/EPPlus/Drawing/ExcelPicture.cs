            ...
            base(drawings, node, NamespacePrefixes[(int)DrawingsType] + ":pic/", NamespacePrefixes[(int)DrawingsType] + ":nvPicPr/" + NamespacePrefixes[(int)DrawingsType] + ":cNvPr", null, DrawingsType)
        {
            ...
            switch (DrawingsType)
            {
                case DrawingsCollectionType.Chart:
                    ...
                    frmXPosition = new ExcelDrawingCoordinate(drawings.NameSpaceManager, offNode, GetPositionSize);
                    frmXSize = new ExcelDrawingSize(drawings.NameSpaceManager, extNode, GetPositionSize, 1);
                    break;
